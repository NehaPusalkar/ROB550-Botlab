...
void ObstacleDistanceGrid::initializeDistances(const OccupancyGrid& map)
{
    ...
    for(cell.y = 0; cell.y < height; ++cell.y)
    {
        for(cell.x = 0; cell.x < width; ++cell.x)
        {
            ...
            {
                //set free cells to 9999 to indicate we have not set a distance yet
                distance(cell.x, cell.y) = 100;
            }
                 ...
                 std::priority_queue<DistanceNode>& searchQueue)
{
    ...
    for(int n = 0; n < 4; ++n)
    {
        ...
        if(grid.isCellInGrid(adjacentCell.x, adjacentCell.y))
        {
            ...
            if(grid(adjacentCell.x, adjacentCell.y) == 100)
            {
                adjacentNode.distance = node.distance + 0.1f;
                // std::cout<<"adj node dist"<<adjacentNode.distance<<std::endl;
